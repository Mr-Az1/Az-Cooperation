	...
	if(mod==2){
		...
		if(isdigit(op)&&num>=1&&num<=4){
			if(num==1){
				...
				if(energy>=t.info){energy-=t.info;t.Info=1;print("基本信息已解锁!");}
				else {print("能量不足!");}
			}
			if(num==3){
				if(t.Weapon==1){print("E.G.O 武器信息已经解锁过了!");} 
				if(energy>=t.vwea){energy-=t.vwea;t.Weapon=1;print("E.G.O 武器信息已解锁!");}
				else{print("能量不足!");}
			}
			if(num==4){
				if(t.Armor==1){print("E.G.O 防具信息已经解锁过了!");} 
				if(energy>=t.var){energy-=t.var;t.Armor=1;print("E.G.O 防具信息已解锁!");}
				else {print("能量不足!");}
			}
